...
import org.eu.hanana.mc.mcef2.cef.dev.DevToolsWsClient;
 ...
 */
public class CefBrowserMC  extends CefBrowser_N implements CefRenderHandler , Closeable {
    ...
    public DevSearchItem getDevToolsData(){
        List<DevSearchItem> list = CefUtil.getDevSearchResults().stream().filter(devSearchItem -> devSearchItem.title.equals(this.title) && devSearchItem.url.equals(this.getURL())).toList();
        for (DevSearchItem devSearchItem : list) {
            try (var dtc = new DevToolsWsClient(devSearchItem.webSocketDebuggerUrl)) {
                String timeout = dtc.getBrowserId().exceptionally(throwable -> "err").completeOnTimeout("timeout", 3, TimeUnit.SECONDS).get();
                if (timeout.equals(String.valueOf(hashCode()))) {
                    return devSearchItem;
                }else if (timeout.equals("err")) {
                    DevSearchItem devSearchItem1 = new DevSearchItem();
                    devSearchItem1.devtoolsFrontendUrl="classpath://cef_test/error.html?error=Operation Failed";
                    return devSearchItem1;
                }else if (timeout.equals("timeout")) {
                    DevSearchItem devSearchItem1 = new DevSearchItem();
                    devSearchItem1.devtoolsFrontendUrl="classpath://cef_test/error.html?error=Operation Time Out";
                    return devSearchItem1;
                }
    ...
    protected CefBrowserMC createDevToolsBrowser(CefClient client, String url, CefRequestContext context, CefBrowser_N parent, Point inspectAt) {
        ...
        CefBrowserMC browserMC = new CefBrowserMC(client, getDevToolsData().devtoolsFrontendUrl, false, context, render){
            @Override
            public void createImmediately() {
        ...
        render.addTask(browserMC::reload);
...
//        log.info("onJSDialog,{}",messageText);
        ...
        if (popup) {
            ...
            synchronized (popupData) {
                ...
                if (popupData.buffer == null || popupData.buffer.capacity() != size)
                    popupData.buffer = BufferUtils.createByteBuffer(size);
    ...
    public void keyEventByKeyCode(int keyCode, int scancode, int mods, boolean pressed) {
        int cefKeyCode = remapKeycode(keyCode, (char) scancode); // 只用 keyCode 做 CEF keyCode 映射
